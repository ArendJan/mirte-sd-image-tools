...
source "arm-image" "mirte_sbc" {
  image_type = "armbian"
  iso_url = var.image_url # not built by CI, but downloaded and uploaded from arbian archives.
  iso_checksum = var.image_checksum
  output_filename = "./workdir/${var.image_name}.img"
  target_image_size = 15*1024*1024*1024 # 15GB
  image_arch = "arm64"
}

build {
  sources = ["source.arm-image.mirte_sbc"]
  provisioner "file" {
    source = "git_local"
